$(() => {
    ...
    $("#nip").change(function () {
        $.ajax({
            method: 'get',
            url: '<?= base_url() ?>sekolah/cari/cekNip',
            data: {
                nip: this.value
            },
        }).done((data) => {
            if (data.data > 0) {
                setToast({ title: "Gagal", body: 'NIP Sudah Terdaftar', class: "bg-warning" });
                this.value = '';
                this.focus();
            }
        }).fail(($xhr) => {
    ...
    $("#form-registrasi").submit(function (e) {
        ...
        $.ajax({
            method: 'post',
            url: '<?= base_url() ?>registrasi/insert_guru',
            data: {
                nip: $('#nip').val(),
                nama: $('#nama').val(),
            ...
            if (data.data > 0) {
                setToast({ title: "Berhasil", body: "Registrasi berhasil. Silahakan tunggu akun di konfirmasi oleh guru administrator.", class: "bg-info" });
            }
            $('#nip').val('');
            $('#nama').val('');
            $('#tanggal_lahir').val('');
